  ...
  if (result) {
    fs.writeFile(
      path.resolve(
        migrationsFolder,
        "../test/",
        config.filename.replace(/\.wasm$/, ".txt")
      ),
      scTask.scAddr,
      "utf8",
      function(err) {
    ...
    console.log(
      'Something went wrong deploying Secret Contract "${contract}", aborting'
    );
    process.exit();
  }
}
...
module.exports = async function(deployer, network, accounts) {
  ...
  await deployer.deploy(Deposit).then(function() {
    console.log(
      `Smart Contract "Deposit.Sol" has been deployed at ETH address: ${Deposit.address}`
    );
    return;
  });
  ...
  const secretConfig = {
    ...
    args: [[owner, "address"], [Deposit.address, "address"]],
    gasLimit: 2000000,
  ...
  console.log(
    `Secret Contract "${secretConfig.filename}" deployed at Enigma address: ${secretAddress}`
  );
};
