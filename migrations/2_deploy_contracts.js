...
module.exports = async function (deployer, network, accounts) {
  ...
  await deployer.deploy(NFT).then(function () {
    console.log(`Smart Contract "NFT.Sol" has been deployed at ETH address: ${NFT.address}`);
    return;
  });

  await deployer.deploy(Deposit).then(function () {
    console.log(`Smart Contract "Deposit.Sol" has been deployed at ETH address: ${Deposit.address}`);
    return;
  });

  const secretConfig = {
    filename: 'lottery.wasm',
    fn: `construct(address, address)`,
    args: [
      [owner, 'address'],
  ...
  const secretAddress = await deploySecretContract(secretConfig);
  console.log(`Secret Contract "${secretConfig.filename}" deployed at Enigma address: ${secretAddress}`);

  const depositContract = await Deposit.at(Deposit.address);
  await depositContract.setSecretContract(enigmaAddr);
  console.log("set secret address to deposit contract");
};